...
{   
    ...
    struct join
    {
        ...
        template <typename Iterable, typename Value = typename Iterable::value_type>
        Value operator()(Iterable&& src) &&
        {
            //static_assert(std::is_same<Ret, decltype(*src.begin())>::value,
            //        "Type of Init must be the same as the return-type of F");
            std::ostringstream os;
            auto b = src.begin(), e = src.end();

            if (b != e) {
                std::copy(b, prev(e), std::ostream_iterator<Value>(os, del));
                b = prev(e);
            }
            if (b != e) {
                os << *b;
        ...
        // See NB[3]
        template<typename Gen, typename Value = typename Gen::value_type>
        Value operator()(seq<Gen> src) &&
        {
            std::ostringstream os;

            auto first = src.get_gen()();
