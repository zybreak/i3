...
static struct injected_reply getmonitors_reply = {nullptr, 0};
static struct injected_reply getoutputinfo_reply = {nullptr, 0};
...
static size_t readall_into(void *buffer, const size_t len, int fd) {
    ...
    while (read_bytes < len) {
        ssize_t n = read(fd, static_cast<char*>(buffer) + read_bytes, len - read_bytes);
        if (n <= 0) {
            return n;
        }
        read_bytes += (size_t)n;
...
static void read_client_setup_request_cb(EV_P_ ev_io *w, int revents) {
    ...
    char *host;
    int displayp;
    if (xcb_parse_display(getenv("DISPLAY"), &host, &displayp, nullptr) == 0) {
...
static void read_client_x11_packet_cb(EV_P_ ev_io *w, int revents) {
    ...
    if (len > sizeof(generic_x11_request_t)) {
        request = (generic_x11_request_t*)srealloc(request, len);
    ...
    if (opcode == XCB_QUERY_EXTENSION) {
        xcb_query_extension_request_t *req = (xcb_query_extension_request_t*)request;
        const char *name = (char*)(request + sizeof(xcb_query_extension_request_t));
...
static bool handle_sequence(struct connstate *connstate, uint16_t sequence) {
    ...
    if (sequence == connstate->getmonitors) {
        ...
        if (getmonitors_reply.buf != nullptr) {
    ...
    if (sequence == connstate->getoutputinfo) {
        ...
        if (getoutputinfo_reply.buf != nullptr) {
            printf("injecting reply\n");
    ...
    switch (((generic_x11_reply_t *)packet)->code) {
        ...
        case 1:  // reply
        {
            ...
            if (len > 32) {
                ...
                must_read(readall_into(static_cast<char*>(packet) + 32, len - 32, connstate->serverw->fd));
            }
            ...
            if (sequence == connstate->getext_randr) {
                xcb_query_extension_reply_t *reply = (xcb_query_extension_reply_t *) packet;
...
int main(int argc, char *argv[]) {
    static struct option long_options[] = {
        {"getmonitors_reply", required_argument, nullptr, 0},
        {"getoutputinfo_reply", required_argument, nullptr, 0},
        {nullptr, 0, nullptr, 0},
    };
