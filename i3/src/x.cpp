...
CIRCLEQ_HEAD(state_head, con_state) state_head = {((struct con_state *) (&state_head)), ((struct con_state *) (&state_head))};

CIRCLEQ_HEAD(old_state_head, con_state) old_state_head = {((struct con_state *) (&old_state_head)), ((struct con_state *) (&old_state_head))};
...
void x_con_init(Con *con) {
    ...
    (state)->state.cqe_next = (&state_head)->cqh_first;
    (state)->state.cqe_prev = ((struct con_state *) (&state_head));
    if ((&state_head)->cqh_last == ((void *) (&state_head)))
        (&state_head)->cqh_last = (state);
    else(&state_head)->cqh_first->state.cqe_prev = (state);
    (&state_head)->cqh_first = (state);

    // CIRCLEQ_INSERT_HEAD(&old_state_head, state, old_state);
    (state)->old_state.cqe_next = (&old_state_head)->cqh_first;
    (state)->old_state.cqe_prev = ((struct con_state *) (&old_state_head));
    if ((&old_state_head)->cqh_last == ((void *) (&old_state_head)))
        (&old_state_head)->cqh_last = (state);
    else(&old_state_head)->cqh_first->old_state.cqe_prev = (state);
    (&old_state_head)->cqh_first = (state);
...
void x_draw_decoration(Con *con) {
    ...
    if (p->border_style != BS_NONE && p->con_is_leaf) {
         ...
         * container otherwise. */
        Rect br = con_border_style_rect(con);
        if (TAILQ_NEXT(con, nodes) == nullptr &&
            TAILQ_PREV(con, nodes_head, nodes) == nullptr &&
    ...
    struct i3Window *win = con->window;
    ...
    if (win == nullptr) {
        ...
        title = con->title_format == nullptr ? win->name : con_parse_title_format(con);
    }
    if (title == nullptr) {
        draw_util_copy_surface(&(con->frame_buffer), &(con->frame), 0, 0, 0, 0, con->rect.width, con->rect.height);
        return;
    ...
    if (win == nullptr || con->title_format != nullptr) {
        I3STRING_FREE(title);
    }
...
void x_push_node(Con *con) {
    ...
    if ((is_pixmap_needed && con->frame_buffer.id == XCB_NONE) || (!rect_equals(state->rect, rect) &&
        ...
        if (is_pixmap_needed && (has_rect_changed || con->frame_buffer.id == XCB_NONE)) {
            ...
            xcb_change_gc(conn, con->frame_buffer.gc, XCB_GC_GRAPHICS_EXPOSURES, value_list);
...
void x_push_changes(Con *con) {
    ...
    if (warp_to) {
        xcb_query_pointer_reply_t *pointerreply = xcb_query_pointer_reply(conn, pointercookie, nullptr);
        if (!pointerreply) {
            ...
            if (current != target) {
                ...
                xcb_change_window_attributes(conn, root, XCB_CW_EVENT_MASK, event_mask_value_list);
    ...
    CIRCLEQ_FOREACH (state, &state_head, state) {
        CIRCLEQ_REMOVE(&old_state_head, state, old_state);
        (state)->old_state.cqe_next = reinterpret_cast<con_state *>(&old_state_head);
        (state)->old_state.cqe_prev = (&old_state_head)->cqh_last;
        if ((&old_state_head)->cqh_first == ((void *) (&old_state_head)))
            (&old_state_head)->cqh_first = (state);
        else(&old_state_head)->cqh_last->old_state.cqe_next = (state);
        (&old_state_head)->cqh_last = (state);
    }
...
void x_raise_con(Con *con) {
    ...
    CIRCLEQ_REMOVE(&state_head, state, state);
    state->state.cqe_next = (&state_head)->cqh_first;
    state->state.cqe_prev = reinterpret_cast<con_state *>(&state_head);
    if ((&state_head)->cqh_last == ((void *) (&state_head))) {
        (&state_head)->cqh_last = (state);
    } else {
        (&state_head)->cqh_first->state.cqe_prev = (state);
    }
    (&state_head)->cqh_first = (state);
}
...
void x_set_i3_atoms() {
    ...
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, root, A_I3_SOCKET_PATH, A_UTF8_STRING, 8,
                        (current_socketpath == nullptr ? 0 : strlen(current_socketpath)),
                        current_socketpath);
