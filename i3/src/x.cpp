 ...
 *
 */
void X::con_init(Con *con, std::optional<xcb_drawable_t> id) {
...
static xcb_drawable_t _x_con_kill(Con *con) {
    ...
    if (con->frame) {
        ...
        if (frame_id == global.x->focused_id) {
...
static Colortriple* get_color(Con *con, bool leaf) {
    ...
    if (con->urgent) {
        return &global.configManager->config->client.urgent;
    } else if (con == global.focused || con->con_inside_focused()) {
        return &global.configManager->config->client.focused;
    } else if (con == con::first(parent->focused)) {
        if (global.configManager->config->client.got_focused_tab_title && !leaf && con_descend_focused(con) == global.focused) {
            /* Stacked/tabbed parent of focused container */
            return &global.configManager->config->client.focused_tab_title;
        } else {
            return &global.configManager->config->client.focused_inactive;
        }
    } else {
        return &global.configManager->config->client.unfocused;
    }
}
...
void x_draw_decoration(Con *con) {
    ...
    p->background = global.configManager->config->client.background;
    ...
    surface_t *dest_surface = nullptr;
    if (con_draw_decoration_into_frame(con)) {
        DLOG(fmt::sprintf("using con->frame_buffer (for con->name=%s) as dest_surface\n", con->name));
        dest_surface = con->frame_buffer ? con->frame_buffer.get() : nullptr;
    } else {
        dest_surface = parent->frame_buffer ? parent->frame_buffer.get() : nullptr;
    ...
    int icon_offset_x;
    int title_offset_x;
    switch (global.configManager->config->title_align) {
        ...
        case title_align_t::ALIGN_CENTER:
             ...
             * padding, text, marks). */
            icon_offset_x = std::max(icon_padding, (deco_width - icon_padding - icon_size - global.configManager->config->font->predict_text_width(title) - title_padding - mark_width) / 2);
        ...
        case title_align_t::ALIGN_RIGHT:
             ...
             *                           ^           ^--- icon_offset_x
             *                           ^--- title_offset_x */
            title_offset_x = std::max(title_padding + mark_width, deco_width - title_padding - global.configManager->config->font->predict_text_width(title) - total_icon_space);
            /* Make sure the icon does not escape title boundaries */
            icon_offset_x = std::min(deco_width - icon_size - icon_padding - title_padding, title_offset_x + global.configManager->config->font->predict_text_width(title) + icon_padding);
            break;
        default:
            ELOG(fmt::sprintf("BUG: invalid config.title_align value %d\n", std::to_underlying(global.configManager->config->title_align)));
            return;
    }
...
void x_push_node(Con *con) {
    ...
    if ((is_pixmap_needed && !con->frame_buffer) || (state->rect != rect &&
        ...
        if (!is_pixmap_needed && con->frame_buffer) {
            auto id = con->frame_buffer->id;
            con->frame_buffer.reset();
            xcb_free_pixmap(**global.x, id);
        }

        if (is_pixmap_needed && (has_rect_changed || !con->frame_buffer)) {
            if (con->frame_buffer) {
                auto id = con->frame_buffer->id;
                con->frame_buffer.reset();
                xcb_free_pixmap(**global.x, id);
            ...
            auto id = xcb_generate_id(**global.x);
...
void x_set_warp_to(std::optional<Rect> rect) {
    if (global.configManager->config->mouse_warping != POINTER_WARPING_NONE) {
