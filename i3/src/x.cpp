...
void x_draw_decoration(Con *con) {
    ...
    if (con->window != nullptr) {
        ...
        /* top area */
        draw_util_rectangle(&(con->frame_buffer), global.configManager->config->client.background,
                            0, 0, r->width, w->y);
        /* bottom area */
        draw_util_rectangle(&(con->frame_buffer), global.configManager->config->client.background,
    ...
    switch (global.configManager->config->title_align) {
        ...
        case title_align_t::ALIGN_CENTER:
             ...
             * padding, text, marks). */
            icon_offset_x = std::max(icon_padding, (deco_width - icon_padding - icon_size - predict_text_width(global.configManager->config->font.get(), **global.x, global.x->root_screen, title) - title_padding - mark_width) / 2);
        ...
        case title_align_t::ALIGN_RIGHT:
             ...
             *                           ^           ^--- icon_offset_x
             *                           ^--- title_offset_x */
            title_offset_x = std::max(title_padding + mark_width, deco_width - title_padding - predict_text_width(global.configManager->config->font.get(), **global.x, global.x->root_screen, title) - total_icon_space);
            /* Make sure the icon does not escape title boundaries */
            icon_offset_x = std::min(deco_width - icon_size - icon_padding - title_padding, title_offset_x + predict_text_width(global.configManager->config->font.get(), **global.x, global.x->root_screen, title) + icon_padding);
            break;
        default:
