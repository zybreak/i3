...
bool match_is_empty(Match *match) {
    ...
    return (match->title == nullptr &&
            match->application == nullptr &&
            match->window_class == nullptr &&
            match->instance == nullptr &&
            match->window_role == nullptr &&
            match->workspace == nullptr &&
            match->machine == nullptr &&
            match->urgent == U_DONTCHECK &&
            match->id == XCB_NONE &&
            match->window_type == UINT32_MAX &&
    ...
    if (match->window_mode != WM_ANY) {
        ...
        switch (match->window_mode) {
            ...
            case WM_TILING_USER:
                ...
                break;
            case WM_TILING:
                if (con_inside_floating(con) != nullptr) {
                    return false;
                }
                break;
...
void match_parse_property(Match *match, const char *ctype, const char *cvalue) {
    ...
    if (strcmp(ctype, "class") == 0) {
        regex_free(match->window_class);
        match->window_class = regex_new(cvalue);
        return;
    }
