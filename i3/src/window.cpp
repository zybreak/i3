...
void window_update_name(i3Window *win, xcb_get_property_reply_t *prop) {
    ...
    if (con != nullptr && con->title_format != nullptr) {
...
bool window_update_normal_hints(i3Window *win, xcb_get_property_reply_t *reply, xcb_get_geometry_reply_t *geom) {
    ...
    if (reply != nullptr) {
        ...
        success = xcb_icccm_get_wm_normal_hints_reply(conn, xcb_icccm_get_wm_normal_hints_unchecked(conn, win->id), &size_hints, nullptr);
    }
    if (!success) {
    ...
    if ((size_hints.flags & XCB_ICCCM_SIZE_HINT_P_MIN_SIZE)) {
        ...
        if (win->min_width != size_hints.min_width) {
            win->min_width = size_hints.min_width;
            changed = true;
        }
        if (win->min_height != size_hints.min_height) {
            win->min_height = size_hints.min_height;
            changed = true;
        }
    }
    ...
    if ((size_hints.flags & XCB_ICCCM_SIZE_HINT_P_MAX_SIZE)) {
        ...
        if (win->max_width != max_width) {
            win->max_width = max_width;
            changed = true;
        }
        if (win->max_height != max_height) {
    ...
    if ((size_hints.flags & XCB_ICCCM_SIZE_HINT_P_RESIZE_INC)) {
        ...
        if (size_hints.width_inc > 0 && size_hints.width_inc < 0xFFFF) {
            if (win->width_increment != size_hints.width_inc) {
                win->width_increment = size_hints.width_inc;
                changed = true;
            }
        } else {
            if (win->width_increment != 0) {
                win->width_increment = 0;
                changed = true;
            }
        }

        if (size_hints.height_inc > 0 && size_hints.height_inc < 0xFFFF) {
            if (win->height_increment != size_hints.height_inc) {
                win->height_increment = size_hints.height_inc;
                changed = true;
            }
        } else {
            if (win->height_increment != 0) {
                win->height_increment = 0;
                changed = true;
            }
        }
    } else {
    ...
    if (size_hints.flags & XCB_ICCCM_SIZE_HINT_BASE_SIZE &&
        ...
        if (win->base_width != size_hints.base_width) {
            win->base_width = size_hints.base_width;
            changed = true;
        }
        if (win->base_height != size_hints.base_height) {
            win->base_height = size_hints.base_height;
            changed = true;
        }
    } else {
    ...
    if (size_hints.flags & XCB_ICCCM_SIZE_HINT_P_ASPECT &&
        ...
        if (win->min_aspect_ratio != 0.0) {
            win->min_aspect_ratio = 0.0;
            changed = true;
        }
        if (win->max_aspect_ratio != 0.0) {
            win->max_aspect_ratio = 0.0;
            changed = true;
        }
    }
...
void window_update_icon(i3Window *win, xcb_get_property_reply_t *prop) {
    ...
    uint64_t len = 0;
    const auto pref_size = (uint32_t)(render_deco_height() - logical_px(2));
