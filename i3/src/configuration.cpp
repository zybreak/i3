...
void INIT_COLOR(Colortriple &x, const char *cborder, const char *cbackground, const char *ctext, const char *cindicator) {
    x.border = draw_util_hex_to_color(global.conn, global.root_screen, cborder);
    x.background = draw_util_hex_to_color(global.conn, global.root_screen, cbackground);
    x.text = draw_util_hex_to_color(global.conn, global.root_screen, ctext);
    x.indicator = draw_util_hex_to_color(global.conn, global.root_screen, cindicator);
    x.child_border = draw_util_hex_to_color(global.conn, global.root_screen, cbackground);
}
...
bool load_configuration(const std::string *override_configpath, config_load_t load_type) {
    ...
    config.client.background = draw_util_hex_to_color(global.conn, global.root_screen, "#000000");
    ...
    if (config.font.type == i3Font::FONT_TYPE_NONE && load_type != config_load_t::C_VALIDATE) {
        ...
        config.font = load_font(global.conn, global.root_screen, "fixed", true);
    ...
    if (load_type == config_load_t::C_RELOAD) {
        ...
        regrab_all_buttons(global.conn);
