...
CFGFUN(include, const char *pattern) {
    ...
    for (size_t i = 0; i < p.we_wordc; i++) {
        ...
        bool skip = false;
        char *file;
        auto it = included_files->begin();
        for (it = included_files->begin(); it != included_files->end(); ++it) {
            file = *it;
            if (strcmp(file, resolved_path) == 0) {
                skip = true;
                break;
            }
        }
...
CFGFUN(exec, const char *exectype, const char *no_startup_id, const char *command) {
    auto *n = (Autostart*)smalloc(sizeof(struct Autostart));
    n->command = sstrdup(command);
    n->no_startup_id = (no_startup_id != nullptr);
    if (strcmp(exectype, "exec") == 0) {
        TAILQ_INSERT_TAIL(&autostarts, n, autostarts);
    } else {
        TAILQ_INSERT_TAIL(&autostarts_always, n, autostarts_always);
    }
}
...
CFGFUN(for_window, const char *command) {
    ...
    auto *assignment = (Assignment*)scalloc(1, sizeof(Assignment));
...
CFGFUN(workspace, const char *workspace, const char *output) {
    ...
    assignment = (Workspace_Assignment*)scalloc(1, sizeof(struct Workspace_Assignment));
...
CFGFUN(bar_output, const char *output) {
    ...
    current_bar->outputs = (char**)srealloc(current_bar->outputs, sizeof(char *) * new_outputs);
...
static void bar_configure_binding(const char *button, const char *release, const char *command) {
    ...
    auto *new_binding = (struct Barbinding*)scalloc(1, sizeof(struct Barbinding));
...
CFGFUN(bar_tray_output, const char *output) {
    auto *tray_output = (struct tray_output_t*)scalloc(1, sizeof(struct tray_output_t));
...
CFGFUN(bar_start) {
    current_bar = (struct Barconfig*)scalloc(1, sizeof(struct Barconfig));
...
CFGFUN(bar_finish) {
    ...
    barconfigs->push_back(current_bar);
