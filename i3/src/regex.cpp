...
struct regex *regex_new(const char *pattern) {
    ...
    auto *re = (struct regex*)scalloc(1, sizeof(struct regex));
    ...
    options |= PCRE_UCP;
    while (!(re->regex = pcre_compile2(pattern, options, &errorcode, &error, &offset, nullptr))) {
...
bool regex_matches(struct regex *regex, const char *input) {
     ...
     * string in bytes */
    if ((rc = pcre_exec(regex->regex, regex->extra, input, strlen(input), 0, 0, nullptr, 0)) == 0) {
