...
static void startup_timeout(EV_P_ ev_timer *w, int revents) {
    ...
    struct Startup_Sequence *current, *sequence = nullptr;
...
void start_application(const char *command, bool no_startup_id) {
    ...
    if (!no_startup_id) {
        ...
        auto *timeout = (struct ev_timer*)scalloc(1, sizeof(struct ev_timer));
        ...
        auto *sequence = (struct Startup_Sequence*)scalloc(1, sizeof(struct Startup_Sequence));
...
struct Startup_Sequence *startup_sequence_get(i3Window *cwindow,
    ...
    if (startup_id_reply == nullptr || xcb_get_property_value_length(startup_id_reply) == 0) {
        ...
        startup_id_reply = xcb_get_property_reply(conn, cookie, nullptr);
