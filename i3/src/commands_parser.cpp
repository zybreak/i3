...
static void push_string(struct stack *stack, const char *identifier, char *str) {
    for (int c = 0; c < 10; c++) {
        if (stack->stack[c].identifier != nullptr)
            continue;
        /* Found a free slot, let’s store it here. */
        stack->stack[c].identifier = identifier;
        stack->stack[c].val.str = str;
        stack->stack[c].type = stack_entry::STACK_STR;
        return;
    }
...
static void push_long(struct stack *stack, const char *identifier, long num) {
    for (int c = 0; c < 10; c++) {
        ...
        stack->stack[c].type = stack_entry::STACK_LONG;
        return;
    }
...
static void clear_stack(struct stack *stack) {
    for (int c = 0; c < 10; c++) {
        if (stack->stack[c].type == stack_entry::STACK_STR)
            free(stack->stack[c].val.str);
        stack->stack[c].identifier = nullptr;
        stack->stack[c].val.str = nullptr;
...
static void next_state(const cmdp_token *token) {
    if (token->next_state == __CALL) {
        ...
        state = (cmdp_state)subcommand_output.next_state;
...
CommandResult *parse_command(const char *input, yajl_gen gen, ipc_client *client) {
    DLOG("COMMAND: *%.4000s*\n", input);
    state = INITIAL;
    auto *result = (CommandResult*)scalloc(1, sizeof(CommandResult));
...
// TODO: make this testable
        ...
        if (!token_handled) {
            ...
            *tokenwalk = '\0';
            ...
            char *position = (char*)smalloc(len + 1);
