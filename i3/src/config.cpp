...
char *current_configpath = nullptr;
char *current_config = nullptr;
Config config;
std::vector<Mode*> *modes = new std::vector<Mode*>;
std::vector<Barconfig*> *barconfigs = new std::vector<Barconfig*>;
std::vector<char*> *included_files = new std::vector<char*>();
...
static void free_configuration() {
    ...
    /* Clear bar configs */
    Barconfig *barconfig;
    while (!barconfigs->empty()) {
        barconfig = *barconfigs->begin();
        FREE(barconfig->id);
        ...
        barconfigs->erase(barconfigs->begin());
        FREE(barconfig);
    }
...
bool load_configuration(const char *override_configpath, config_load_t load_type) {
    ...
    auto *default_mode = (struct Mode*)scalloc(1, sizeof(struct Mode));
    default_mode->name = sstrdup("default");
    default_mode->bindings = (struct bindings_head*)scalloc(1, sizeof(struct bindings_head));
...
#define INIT_COLOR(x, cborder, cbackground, ctext, cindicator) \
    ...
    while (!included_files->empty()) {
        auto begin = included_files->begin();
        FREE(*begin);
        included_files->erase(begin);
    }

    char resolved_path[PATH_MAX] = {'\0'};
    if (realpath(current_configpath, resolved_path) == nullptr) {
        die("realpath(%s): %s", current_configpath, strerror(errno));
    }

    char *file = sstrdup(resolved_path);
    ...
    if (config.font.type == i3Font::FONT_TYPE_NONE && load_type != C_VALIDATE) {
