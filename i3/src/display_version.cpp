...
void display_running_version() {
    ...
    int sockfd = ipc_connect(nullptr);
    ...
    yajl_handle handle = yajl_alloc(&version_callbacks, nullptr, nullptr);
    ...
    if (loaded_config_file_name) {
        ...
        for (auto file : *included_files) {
            print_config_path(file, "included");
        }
    }
    ...
    if (strstr(destpath, "(deleted)") != nullptr)
