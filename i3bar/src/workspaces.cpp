...
static int workspaces_boolean_cb(void *params_, int val) {
    auto *params = (struct workspaces_json_params *)params_;
...
static int workspaces_start_map_cb(void *params_) {
    ...
    i3_ws *new_workspace = nullptr;

    if (params->cur_key == nullptr) {
        new_workspace = (i3_ws*)smalloc(sizeof(i3_ws));
        new_workspace->num = -1;
        new_workspace->name = nullptr;
        new_workspace->visible = false;
        new_workspace->focused = false;
        new_workspace->urgent = false;
        memset(&new_workspace->rect, 0, sizeof(rect));
        new_workspace->output = nullptr;
...
void parse_workspaces_json(char *json) {
    ...
    params.workspaces_walk = nullptr;
    params.cur_key = nullptr;
    params.json = json;

    yajl_handle handle;
    yajl_status state;
    handle = yajl_alloc(&workspaces_callbacks, nullptr, (void *)&params);
...
void free_workspaces() {
    ...
    SLIST_FOREACH (outputs_walk, outputs, slist) {
        if (outputs_walk->workspaces != nullptr && !TAILQ_EMPTY(outputs_walk->workspaces)) {
