...
static int outputs_null_cb(void *params_) {
    auto *params = (struct outputs_json_params *)params_;
...
static int outputs_start_map_cb(void *params_) {
    auto *params = (struct outputs_json_params *)params_;
    i3_output *new_output = nullptr;

    if (params->cur_key == nullptr) {
        new_output = (i3_output*)smalloc(sizeof(i3_output));
        new_output->name = nullptr;
        ...
        new_output->workspaces = (struct ws_head*)smalloc(sizeof(struct ws_head));
        TAILQ_INIT(new_output->workspaces);

        new_output->trayclients = (struct tc_head*)smalloc(sizeof(struct tc_head));
 ...
 */
void init_outputs() {
    outputs = (struct outputs_head*)smalloc(sizeof(struct outputs_head));
    SLIST_INIT(outputs);
}
...
void parse_outputs_json(char *json) {
    ...
    params.outputs_walk = nullptr;
    params.cur_key = nullptr;
    params.json = json;
    params.in_rect = false;

    yajl_handle handle;
    yajl_status state;
    handle = yajl_alloc(&outputs_callbacks, nullptr, (void *)&params);
...
void free_outputs() {
    ...
    SLIST_FOREACH (outputs_walk, outputs, slist) {
        ...
        if (outputs_walk->trayclients != nullptr && !TAILQ_EMPTY(outputs_walk->trayclients)) {
