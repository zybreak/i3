...
static char *expand_path(char *path) {
    ...
    if (glob(path, GLOB_NOCHECK | GLOB_TILDE, nullptr, &globbuf) < 0) {
        ELOG("glob() failed\n");
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **argv) {
    ...
    static struct option long_opt[] = {
        ...
        {"bar_id", required_argument, nullptr, 'b'},
    ...
    if (socket_path == nullptr) {
        const char *i3_default_sock_path = "/tmp/i3-ipc.sock";
     ...
     * some calls to free() */
    auto *sig_term = (ev_signal*)smalloc(sizeof(ev_signal));
    auto *sig_int = (ev_signal*)smalloc(sizeof(ev_signal));
    auto *sig_hup = (ev_signal*)smalloc(sizeof(ev_signal));
