...
static void copy_statusline(struct statusline_head *from, struct statusline_head *to) {
    ...
    TAILQ_FOREACH (current, from, blocks) {
        auto *new_block = (status_block*)smalloc(sizeof(struct status_block));
...
__attribute__((format(printf, 1, 2))) static void set_statusline_error(const char *format, ...) {
    ...
    auto *err_block = (status_block*)scalloc(1, sizeof(struct status_block));
    err_block->full_text = i3string_from_utf8("Error: ");
    err_block->name = sstrdup("error");
    err_block->color = sstrdup("#ff0000");
    err_block->no_separator = true;

    auto *message_block = (status_block*)scalloc(1, sizeof(struct status_block));
...
static int stdin_end_map(void *context) {
    ...
    if (new_block->short_text != nullptr)
...
static unsigned char *get_buffer(ev_io *watcher, int *ret_buffer_len) {
    ...
    auto *buffer = (unsigned char *)smalloc(buffer_len + 1);
    buffer[0] = '\0';
    while (1) {
        ...
        if (rec == buffer_len) {
            ...
            buffer = (unsigned char *)srealloc(buffer, buffer_len);
        }
    }
    if (*buffer == '\0') {
        FREE(buffer);
...
static void stdin_io_first_line_cb(struct ev_loop *loop, ev_io *watcher, int revents) {
    ...
    if (child.version > 0) {
        ...
        auto *new_block = (status_block*)scalloc(1, sizeof(struct status_block));
...
static void child_write_output() {
...
void start_child(char *command) {
    ...
    parser = yajl_alloc(&callbacks, nullptr, &parser_context);
    ...
    switch (child.pid) {
        ...
        case 0:
            ...
            setpgid(child.pid, 0);
            execl(_PATH_BSHELL, _PATH_BSHELL, "-c", command, (char *)nullptr);
            return;
        default:
    ...
    stdin_io = (ev_io*)smalloc(sizeof(ev_io));
    ev_io_init(stdin_io, &stdin_io_first_line_cb, stdin_fd, EV_READ);
    ev_io_start(main_loop, stdin_io);

    /* We must cleanup, if the child unexpectedly terminates */
    child_sig = (ev_child*)smalloc(sizeof(ev_child));
...
static void child_click_events_initialize() {
    DLOG_CHILD;
