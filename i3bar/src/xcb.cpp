...
static xcb_intern_atom_reply_t *tray_reply = nullptr;
...
int _xcb_request_failed(xcb_void_cookie_t cookie, const char *err_msg, int line) {
    ...
    if ((err = xcb_request_check(xcb_connection, cookie)) != nullptr) {
...
static void draw_separator(i3_output *output, uint32_t x, struct status_block *block, bool use_focus_colors) {
    ...
    if (TAILQ_NEXT(block, blocks) == nullptr || sep_offset == 0)
        return;

    uint32_t center_x = x - sep_offset;
    if (config.separator_symbol == nullptr) {
...
static uint32_t predict_statusline_length(bool use_short_text) {
    ...
    TAILQ_FOREACH (block, &statusline_head, blocks) {
        ...
        if (use_short_text && block->short_text != nullptr) {
        ...
        if (TAILQ_NEXT(block, blocks) != nullptr)
...
static void hide_bars() {
    if ((config.hide_on_modifier == M_DOCK) || (config.hidden_state == config_t::S_SHOW && config.hide_on_modifier == M_HIDE)) {
        return;
    }
...
static void child_handle_button(xcb_button_press_event_t *event, i3_output *output, uint32_t statusline_x, uint32_t statusline_y) {
    ...
    TAILQ_FOREACH (block, &statusline_head, blocks) {
        ...
        if (output->statusline_short_text && block->short_text != nullptr) {
            text = block->short_text;
            render = &block->short_render;
        } else {
...
static void handle_button(xcb_button_press_event_t *event) {
    ...
    i3_ws *cur_ws = nullptr, *clicked_ws = nullptr, *ws_walk;
    ...
    TAILQ_FOREACH (ws_walk, walk->workspaces, tailq) {
        ...
        if (ws_walk->visible)
            cur_ws = ws_walk;
        workspace_width += w;
        if (TAILQ_NEXT(ws_walk, tailq) != nullptr)
    ...
    char *buffer = (char*)scalloc(len + num_quotes, 1);
...
static void configure_trayclients() {
    ...
    SLIST_FOREACH (output, outputs, slist) {
        ...
        int idx = 0;
        auto **trayclients = (trayclient**)smalloc(count * sizeof(trayclient *));
        TAILQ_FOREACH (client, output->trayclients, tailq) {
            if (client->mapped) {
...
static trayclient *trayclient_from_window(xcb_window_t win) {
    return trayclient_and_output_from_window(win, nullptr);
}
...
static void handle_client_message(xcb_client_message_event_t *event) {
    if (event->type == atoms[I3_SYNC]) {
        ...
        if (op == SYSTEM_TRAY_REQUEST_DOCK) {
            ...
            if (xembedr != nullptr && xembedr->length != 0) {
                DLOG("xembed format = %d, len = %d\n", xembedr->format, xembedr->length);
                auto *xembed = (uint32_t*)xcb_get_property_value(xembedr);
            ...
            auto *tc = (trayclient*)scalloc(1, sizeof(trayclient));
            tc->win = client;
...
static void xcb_prep_cb(struct ev_loop *loop, ev_prepare *watcher, int revents) {
    ...
    while ((event = xcb_poll_for_event(xcb_connection)) != nullptr) {
...
#include "xcb_atoms.def"
    ...
    visual_type = config.transparency ? xcb_aux_find_visual_by_attrs(root_screen, -1, 32) : nullptr;
    ...
    xcb_io = (ev_io*)smalloc(sizeof(ev_io));
    xcb_prep = (ev_prepare*)smalloc(sizeof(ev_prepare));
 ...
 *
 */
static void register_xkb_keyevents() {
 ...
 *
 */
static void deregister_xkb_keyevents() {
    xcb_xkb_select_events(conn,
                          XCB_XKB_ID_USE_CORE_KBD,
                          0,
                          0,
...
void init_xcb_late(char *fontname) {
    if (fontname == nullptr)
        fontname = (char*)"-misc-fixed-medium-r-normal--13-120-75-75-C-70-iso10646-1";
...
static void send_tray_clientmessage() {
    uint8_t buffer[32] = {0};
    auto *ev = (xcb_client_message_event_t *)buffer;
...
static void init_tray() {
    ...
    if (tray_reply == nullptr) {
        if (!(tray_reply = xcb_intern_atom_reply(xcb_connection, tray_cookie, nullptr))) {
    ...
    if (!(selreply = xcb_get_selection_owner_reply(xcb_connection, selcookie, nullptr))) {
...
void init_tray_colors() {
    ...
    char strgroups[3][3] = {{bar_fg[1], bar_fg[2], '\0'},
                            {bar_fg[3], bar_fg[4], '\0'},
                            {bar_fg[5], bar_fg[6], '\0'}};
    const uint8_t r = strtol(strgroups[0], nullptr, 16);
    const uint8_t g = strtol(strgroups[1], nullptr, 16);
    const uint8_t b = strtol(strgroups[2], nullptr, 16);
...
void kick_tray_clients(i3_output *output) {
    ...
    uint8_t buffer[32] = {0};
    auto *event = (xcb_destroy_notify_event_t *)buffer;
...
void destroy_window(i3_output *output) {
    if (output == nullptr || output->bar.id == XCB_NONE) {
        return;
    }
 ...
 *   primary.
 */
static i3_output *get_tray_output() {
    i3_output *output = nullptr;
...
void reconfig_windows(bool redraw_bars) {
    ...
    SLIST_FOREACH (walk, outputs, slist) {
        ...
        if (walk->bar.id == XCB_NONE) {
            ...
            char *clazz;
            int len = sasprintf(&clazz, "%s%ci3bar%c", config.bar_id, 0, 0);
            ...
            draw_util_surface_init(xcb_connection, &walk->bar, bar_id, nullptr, walk->rect.w, bar_height);
            draw_util_surface_init(xcb_connection, &walk->buffer, buffer_id, nullptr, walk->rect.w, bar_height);
            draw_util_surface_init(xcb_connection, &walk->statusline_buffer, statusline_buffer_id, nullptr, walk->rect.w, bar_height);
            ...
            draw_util_surface_init(xcb_connection, &(walk->bar), walk->bar.id, nullptr, walk->rect.w, bar_height);
            draw_util_surface_init(xcb_connection, &(walk->buffer), walk->buffer.id, nullptr, walk->rect.w, bar_height);
            draw_util_surface_init(xcb_connection, &(walk->statusline_buffer), walk->statusline_buffer.id, nullptr, walk->rect.w, bar_height);
...
void draw_bars(bool unhide) {
    ...
    if (mod_pressed ||
        config.hidden_state == config_t::S_SHOW ||
        unhide) {
        unhide_bars();
...
void set_current_mode(struct mode *current) {
    ...
    binding = *current;
    activated_mode = binding.name != nullptr;
}
