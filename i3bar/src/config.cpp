...
static int config_string_cb(void *params_, const unsigned char *val, size_t _len) {
    ...
    if (parsing_bindings) {
        if (strcmp(cur_key, "command") == 0) {
            binding_t *binding = TAILQ_LAST(&(config.bindings), config_t::bindings_head);
            if (binding == nullptr) {
    ...
    if (parsing_tray_outputs) {
        ...
        auto *tray_output = (tray_output_t*)scalloc(1, sizeof(tray_output_t));
    ...
    if (!strcmp(cur_key, "hidden_state")) {
        ...
        config.hidden_state = (len == strlen("hide") && !strncmp((const char *)val, "hide", strlen("hide")) ? config_t::S_HIDE : config_t::S_SHOW);
        return 1;
    }
    ...
    if (!strcmp(cur_key, "wheel_up_cmd")) {
        DLOG("wheel_up_cmd = %.*s\n", len, val);
        auto *binding = (binding_t*)scalloc(1, sizeof(binding_t));
    ...
    if (!strcmp(cur_key, "outputs")) {
        ...
        config.outputs = (char**)srealloc(config.outputs, sizeof(char *) * new_num_outputs);
...
void parse_config_json(char *json) {
    yajl_handle handle = yajl_alloc(&outputs_callbacks, nullptr, nullptr);
...
void parse_get_first_i3bar_config(char *json) {
    ...
    yajl_handle handle = yajl_alloc(&configs_callbacks, nullptr, nullptr);
