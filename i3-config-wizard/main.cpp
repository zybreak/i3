...
static char *socket_path = nullptr;
...
static void push_string(const char *identifier, const char *str) {
    for (int c = 0; c < 10; c++) {
        ...
        if (stack[c].identifier == nullptr) {
            ...
            stack[c].type = stack_entry::STACK_STR;
        } else {
            /* Append the value. */
...
static void push_long(const char *identifier, long num) {
    for (int c = 0; c < 10; c++) {
        ...
        stack[c].identifier = identifier;
        stack[c].val.num = num;
        stack[c].type = stack_entry::STACK_LONG;
        return;
    }
...
static void clear_stack() {
    for (int c = 0; c < 10; c++) {
        if (stack[c].type == stack_entry::STACK_STR)
            free(stack[c].val.str);
        stack[c].identifier = nullptr;
        stack[c].val.str = nullptr;
        stack[c].val.num = 0;
    }
}
...
static char *next_state(const cmdp_token *token) {
    ...
    if (token->next_state == __CALL) {
        ...
        if (modifiers != nullptr &&
            strstr(modifiers, "Shift") != nullptr) {
        ...
        char *res;
        char *modrep = (modifiers == nullptr ? sstrdup("") : sstrdup(modifiers));
        char *comma;
        while ((comma = strchr(modrep, ',')) != nullptr) {
            *comma = '+';
        }
        sasprintf(&res, "bindsym %s%s%s %s%s\n", (modifiers == nullptr ? "" : modrep), (modifiers == nullptr ? "" : "+"), str, (release == nullptr ? "" : release), get_string("command"));
        clear_stack();
        free(modrep);
        return res;
    }
        ...
        for (c = 0; c < ptr->n; c++) {
            ...
            if (token->name[0] == '\'') {
                if (strncasecmp(walk, token->name + 1, strlen(token->name) - 1) == 0) {
                    ...
                    if ((result = next_state(token)) != nullptr)
                        return result;
                    break;
                }
                continue;
...
static void txt(int col, int row, const char *text, color_t fg, color_t bg) {
    int x = col_x(col);
    int y = row_y(row);
...
static void finish() {
    ...
    if ((xkb_keymap = xkb_x11_keymap_new_from_device(xkb_context, conn, device_id, XKB_KEYMAP_COMPILE_NO_FLAGS)) == nullptr)
        errx(1, "xkb_x11_keymap_new_from_device failed");

    FILE *kc_config = fopen(SYSCONFDIR "/i3/config.keycodes", "r");
    if (kc_config == nullptr)
        err(1, "Could not open input file \"%s\"", SYSCONFDIR "/i3/config.keycodes");

    FILE *ks_config = fopen(config_path, "w");
    if (ks_config == nullptr)
...
int main(int argc, char *argv[]) {
    ...
    const char *patternbold = "pango:monospace bold 8";
    ...
    const char *options_string = "sm:vh";
    ...
    char *path = get_config_path(nullptr, false);
    if (path != nullptr) {
    ...
    int screen;
    if ((conn = xcb_connect(nullptr, &screen)) == nullptr ||
...
#undef xmacro
    ...
    if (!(modmap_reply = xcb_get_modifier_mapping_reply(conn, modmap_cookie, nullptr)))
...
#undef xmacro
    ...
    xcb_grab_keyboard_reply_t *reply = nullptr;

    int count = 0;
    while ((reply == nullptr || reply->status != XCB_GRAB_STATUS_SUCCESS) && (count++ < 500)) {
        cookie = xcb_grab_keyboard(conn, false, win, XCB_CURRENT_TIME, XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC);
        reply = xcb_grab_keyboard_reply(conn, cookie, nullptr);
        usleep(1000);
    }
    ...
    while ((event = xcb_wait_for_event(conn)) != nullptr) {
        ...
        switch (type) {
            case XCB_KEY_PRESS:
                handle_key_press(nullptr, conn, (xcb_key_press_event_t *)event);
                break;
