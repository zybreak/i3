...
int main(int argc, char *argv[]) {
    ...
    static struct option long_options[] = {
        ...
        {"button-no-terminal", required_argument, nullptr, 'B'},
    ...
    std::string options_string = "b:B:f:m:t:vhp";

    prompt = i3string_from_utf8("Please do not run this program.");

    while ((o = getopt_long(argc, argv, options_string.c_str(), long_options, &option_index)) != -1) {
        switch (o) {
            case 'v':
                free(pattern);
            ...
            case 'b':
            case 'B':
                buttons = (button_t*)srealloc(buttons, sizeof(button_t) * (buttoncnt + 1));
